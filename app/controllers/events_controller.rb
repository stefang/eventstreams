  ...
  def show
    if current_subdomain.blank?
      ...
      if @event.blank?
        ...
        @tweets = @event.owned_tweets.find(:all, :limit => t("#{@event.theme_name}.front_page_tweet_count"))
        @latest_news = @event.published_news_items.find(:all, :limit => t("#{@event.theme_name}.front_page_news_count"))
        render :layout => 'event'
      end
    end
  end
